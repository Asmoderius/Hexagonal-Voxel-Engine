...
/*
    ...
    Author: Louis Fleron.
    Description:
    Dune biome is raised areas in deserts. Sandy and arid.
 */
public class Dune : Biome
{

    public float caveSize = 0.025f; //Higher value = smaller caves. 
    public float caveFrequency = 5;
    private float ideal_Temperature_Start = 32f;
    private float ideal_Temperature_End = 41f;
    ...
    public override Block GenerateBlock(float noise_X, float noise_Y, float noise_Z, short y, short lowerGroundHeight, short groundHeight, bool topSoil, float temperature, float moisture)
    {
        ...
        if (y <= groundDensity)
        {
            ...
            if (caveFrequency < caveChance)
            {
                ...
                if (topSoil)
                {
                    blockToBuild = Blocks.Get(17);
                }
                else
                {
                    blockToBuild = Blocks.Get(18);
                }
            }
        }
        return blockToBuild;
