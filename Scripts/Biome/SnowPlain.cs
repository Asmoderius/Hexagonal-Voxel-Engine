...
/*
    ...
    Author: Louis Fleron.
    Description:
    Flat snowy areas found in arctic climate.
       
    Problem: 
    Bad name
 */
public class SnowPlain : Biome {
    ...
    public override Block GenerateBlock(float noise_X, float noise_Y, float noise_Z, short y, short lowerGroundHeight, short groundHeight, bool topSoil, float temperature, float moisture)
    {
        ...
        if (y <= groundDensity)
        {
            ...
            if (caveFrequency < caveChance)
            {
                ...
                {
                    blockToBuild = new Block(BlockType.Dirt, false, true, false, false, 2, 150, 100, 0, 255);
                }
                */
            }
        }
